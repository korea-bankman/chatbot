...
# 3. PDF를 읽고 조각내어 저장하는 함수 (구글 API를 쓰지 않아 에러가 나지 않음)
@st.cache_data
...
if prompt := st.chat_input("수혈 지침에 대해 질문해 주세요! (예: 신선동결혈장 투여 기준)"):
    ...
    with st.chat_message("assistant"):
        try:
            # 💡 [핵심 개선] 외부 API 없이 내 서버에서 즉시 핵심 단어를 매칭하는 시스템
            ...
            for chunk in chunks:
                ...
                # 사용자가 입력한 단어들이 문서 조각에 얼마나 자주 포함되어 있는지 계산
            ...
            # 메인 답변을 생성할 최신 제미나이 모델 호출 (안정적인 메인 모델 활용)
